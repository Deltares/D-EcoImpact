...
rule1C = CombineResultsRule("Compute ssc of mud", ["mud1_kgm3", "mud2_kgm3"], MultiArrayOperationType.ADD, "mud_kgm3")
...
rule4A = TimeAggregationRule("Compute mean ssc", ["mud_kgm3"], TimeOperationType.AVERAGE, "year","mean_mud_kgm3")
rule4B = FormulaRule("Determine depth of photic zone", ["mean_mud_kgm3"], "4.605170185988091/(0.08 + 2.5e4 * mud_kgm3)", "dpar1_m")
...
rule4C = FormulaRule("Determine level of photic zone", ["dpar1_m", "MLWN"], "MLWN - dpar1_m", "zpar1_mNAP")
...
rule4E = FormulaRule("Get exposure time", ["mesh2d_Inundation"], "1 - mesh2d_Inundation", "exposure_time")
...
rule6A = StepFunctionRule("Sediment composition class 1 and 2", "mesh2d_mudfrac", [0.0, 0.25, 1.0], [2.0, 1.0, 1.0], "SedCode1_2")
...
rule7A1 = FormulaRule("Correction for 1/0 error in rule 7A2", ["exposure_time"], "(exposure_time == 0) * 0.0001", "correction")
rule7A2 = FormulaRule("Determine expected number of inundations per year", ["mesh2d_Inundation", "correction"], "1 / (1 - mesh2d_Inundation + correction)", "n_inundations")
rule7B = StepFunctionRule("Salt marsh inundation class", "n_inundations", [0.0, 5.0, 50.0, 150, 300, 99999999], [4.0, 3.0, 2.0, 1.0, 0.0, 0.0], "SaltMarshC")
...
model = RuleBasedModel([inputDataset], [rule1A, rule1B, rule1C,\
                                        ...
                                        rule4F, rule4G, rule4H, rule4I,\
                                        rule5A, rule5B,\
                                        rule6A, rule6B, rule6C, rule6D, \
                                        rule7A1, rule7A2, rule7B \
                                        ], name="my new model")
